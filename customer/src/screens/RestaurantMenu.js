  ...
  return (
    <TouchableOpacity
      ...
      <View style={style.iconAndValueContainer}>
        <IconMCI name="cart-outline" size={30} color="#fff" />
        <Text style={style.flagText}>
          Cart value : {GetCurrencySymbolFromId(props.cartValue.currency)}{' '}
          {Dinero(props.cartValue).toUnit()}
        </Text>
      </View>
      <View>
...
function RestaurantMenu(props) {
  ...
  const UpdateFavourite = async val => {
    ...
    if (val) SetFavourite(props.route.params.merchantId);
    else UnsetFavourite(props.route.params.merchantId);
  };

  const loadData = () => {
    setLoading(true);
    GetMerchantInfo(props.route.params.merchantId)
      .then(merch => {
        console.log(merch);
        setData(merch);
        setFav(merch.isFav);
        ...
        if (props.route.params.showItem) {
          const item = liveMenu.filter(
            val => val.id == props.route.params.showItem,
          )[0];
          const finalList = liveMenu.filter(
            item => item.id !== props.route.params.showItem,
          );
  ...
  return (
    <View style={style.inventoryContainer}>
      <ScrollView
        ...
        <View style={style.bannerContainer}>
          ...
          <TouchableOpacity
            style={[
              style.favWrapper,
              isFav ? {backgroundColor: '#d42522'} : {},
            ]}
            activeOpacity={0.8}
            onPress={() => {
              UpdateFavourite(!isFav);
            }}>
            <ANT name="heart" size={16} color={isFav ? '#fff' : '#d42522'} />
          </TouchableOpacity>
        </View>
        ...
        <View style={style.menuContainer}>
          {!loading && listings.length === 0 && (
            <Text style={{fontSize: 24, alignSelf: 'center', margin: 20}}>
              No Items listed
            </Text>
          )}
      ...
      {data.openTill < new Date().getTime() ? (
        <View
          style={[style.flagMessageContainer, {backgroundColor: '#c2130e'}]}>
