...
function Explore(props) {
  ...
  const [sliderYAxis, setSliderYAxis] = useState(
  ...
  const loadSearchResults = () => {
    ...
    SearchNearMe(searchText, seletedFilterTags.join(','), 0)
      .then(val => {
        setRestros(val.data);
      ...
      .then(granted => {
        if (!granted) {
          if (Object.keys(location).length === 0) {
            Alert.alert(
              'Location Needed',
              'Please grant location permission so we could show you restaurants near you',
              [
                {
                  text: 'Grant',
                  style: 'default',
  ...
  useEffect(() => {
    ...
    GetProfile().then(profile => {
      ...
      if (
        ...
        profile.locationPoint.coordinates &&
        profile.locationPoint.coordinates.length === 2
      ) {
        // location already exists hence a general check
        Alert.alert(
          'Location',
          'Update your location to get best deals near you',
          [
            {
        ...
        Alert.alert(
          'Location Needed',
          'We need your location to show offers near you',
          [
            {
              text: 'Update Location',
              onPress: handleUserLocation,
  ...
  return (
    <View style={style.storeContainer}>
      ...
      <View
        ...
        <View
          onTouchStart={() => console.log('Hp')}
          style={style.holderBar}></View>
        ...
        <View
          ...
          <IconMI name="search" size={25} color="#8c8c8c" />
          <TextInput
            value={searchText}
          ...
          {searchText.length > 0 && (
            <TouchableOpacity
              ...
              <IconMI name="close" size={15} color="#fff" />
            </TouchableOpacity>
          )}
        </View>
        ...
        {filterTags.length > 0 && (
          <View style={{width: '100%', height: 60}}>
            <ScrollView
              ...
              horizontal
              contentContainerStyle={style.filterCapsuleContainer}>
              {seletedFilterTags.length > 0 && (
                <TouchableOpacity
                  ...
                  <Text
                    style={[
                      style.capsule,
                      {color: '#d42522', borderColor: '#d42522'},
                    ]}>
                    Clear
                  </Text>
                </TouchableOpacity>
                ...
                return (
                  <TouchableOpacity
                    ...
                    <Text
                      style={[
                        style.capsule,
                        seletedFilterTags.includes(tag)
                          ? style.selectedCapsule
                          : {},
                      ]}>
                      {tag}
                    </Text>
        ...
        <ScrollView
          ...
          {restros.map(restro => {
            ...
            return (
              <RestaurantCard
                ...
                rating={`${
                  restro.rating ? restro.rating.toFixed(1) : 0
                } Stars (${restro.ratingCount ?? 0})`}
          ...
          {!loadingResults && restros.length === 0 && (
            <Text style={style.noResultText}>
              No Restaurants are open near you at the moment
            </Text>
          )}
      ...
      {!loadingResults && restros.length === 0 && (
        <Text style={style.NoRestauraunt}>
          No Restauraunts in this area.{'\n'}Try clearing all filters or come
          after sometime
        </Text>
      )}
      ...
      <View
        onLayout={ev => {
          if (TOP_BAR_Y_OCCUPANCY.current === 0) {
            TOP_BAR_Y_OCCUPANCY.current =
              ev.nativeEvent.layout.y + ev.nativeEvent.layout.height + 10;
          }
        }}
        ...
        <TouchableOpacity
          ...
          <Text numberOfLines={1} style={style.locationText}>
            {GetTrimmedLocation(location.label)}
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
          activeOpacity={0.6}
