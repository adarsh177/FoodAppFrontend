...
function StartStoreDialog(props) {
  ...
  const [isStoreCloseDatePickerVisible, setStoreCloseDatePickerVisibility] =
    useState(false);
  ...
  const StartAccepting = () => {
    ...
    setLoading(true);
    UpdateStoreStatus(storeCloseDate.getTime(), pickupDate.getTime())
      .then(() => {
        close();
      })
      .catch(err => {
    ...
    <Modal
      ...
      <View style={style.mainContainer}>
        <View style={style.inventoryModalContainer}>
          ...
          <TouchableOpacity
            onPress={0.8}
            onPress={() => setStoreCloseDatePickerVisibility(true)}>
            <Text style={style.datetime}>
              {storeCloseDate
                ? `${storeCloseDate.toLocaleTimeString()} ${storeCloseDate.toLocaleDateString()}`
                : `Select date time`}
            </Text>
          </TouchableOpacity>
          <DateTimePickerModal
          ...
          <TouchableOpacity
            ...
            <Text style={style.datetime}>
              {pickupDate
                ? `${pickupDate.toLocaleTimeString()} ${pickupDate.toLocaleDateString()}`
                : `Select date time`}
            </Text>
          </TouchableOpacity>
          <DateTimePickerModal
